  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-4xl max-h-[85vh] glass rounded-2xl border border-white/10 overflow-hidden flex flex-col"
                  ...
                  {filteredPeople.map((person) => (
                    <motion.div
                      ...
                      className={`p-4 cursor-pointer hover:bg-white/5 transition-colors ${
                        selectedPerson?.id === person.id ? "bg-white/10" : ""
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium truncate capitalize">{person.name}</p>
...
function PersonDetails({ person, onEdit, onDelete }: PersonDetailsProps) {
  return (
    <div className="p-6">
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-emerald-500 flex items-center justify-center text-white text-2xl font-bold shadow-lg shadow-blue-500/20">
            {person.name.charAt(0).toUpperCase()}
          </div>
          <div>
            <h3 className="text-xl font-bold capitalize">{person.name}</h3>
            {person.relationship && (
              <span className="inline-block mt-1 px-2 py-0.5 rounded-full text-xs bg-white/10 capitalize">
                {person.relationship}
              </span>
            )}
          </div>
      ...
      <div className="glass rounded-xl p-4 border border-white/10 mb-6">
        ...
        >
          {person.running_balance > 0 && "You owe "}
          {person.running_balance < 0 && "Owes you "}
          {person.running_balance === 0 && "All settled "}
          {person.running_balance !== 0 && formatCurrency(person.running_balance)}
        </p>
      </div>
